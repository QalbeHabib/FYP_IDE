...
int main()
{
  ...
  componentDidMount() {
    ...
    this.props.socket.on("running", (val) => {
      if (val) this.setState({ curstate: "run" });
    });
    this.props.socket.on("exit", (val) => {
      if (val) this.setState({ curstate: "" });
    });
    ...
    if (this.props.match) {
      let program = this.props.programs.find((program) => {
        if (program._id === this.props.match.params.id) return program;
      });
  ...
  updatecpp = (val) => this.setState({ cpp: val });
  render() {
            ...
            <div style={{ width: "100%", padding: "1%", position: "relative" }}>
              ...
              {value === 0 && (
                <MyEditor
                  mode="c_cpp"
                  theme="monokai"
                  update={this.updatecpp}
                  defvalue={this.state.cpp}
                />
              )}
              <div
            ...
            <div style={{ width: "100%", padding: "1%" }}>
              <div style={{ width: "100%", height: "47px", paddingTop: "5px" }}>
                <Button
                  ...
                  color="primary"
                  onClick={
                    (curstate === "" && this.runcpp) ||
                    (curstate === "comp" && this.stopcomp) ||
                    (curstate === "run" && this.stoprun)
                  }
                  style={{ float: "left" }}
                >
                  {(curstate === "" && "run") ||
                    (curstate === "comp" && "Cancel") ||
                    (curstate === "run" && "Stop")}
                </Button>
                <Save
                  type="cpp"
